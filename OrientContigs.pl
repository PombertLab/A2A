...
## Parsing the protein blast file, counting the amount of proteins that are in increasing order (same orientation),
## in decreasing order (opposite orientation), or jumbled order (inconclusive)
...
while (my $line = <BP>){
    ...
    my @info = split("\t",$line);
    my $contig_info = $info[0]; my $accession_info = $info[1];
    ...
    unless($prev_contig){
        $prev_contig = $current_contig; $prev_accession_number = $current_accession_number; next;
    }
